...
window.onload = async function () {
  ...
  const mintingQR = new QRCode('qrMinting', { width: 160, height: 160, correctLevel: QRCode.CorrectLevel.L })
  const transferingQR = new QRCode('qrTransfering', { width: 160, height: 160, correctLevel: QRCode.CorrectLevel.L })
  const editingQR = new QRCode('qrEditing', { width: 160, height: 160, correctLevel: QRCode.CorrectLevel.L })
  ...
  document.getElementById('searchForName').addEventListener('keyup', async function (event) {
    if (event.key === 'Enter') {
      ...
      if (resolvedAddress === 'vite_0000000000000000000000000000000000000000a4f3a0cb58') {
        ...
        mintingQR.makeCode(`vite:${contractAddress}/mint?amount=200&data=${Buffer.from(mintCall, 'hex').toString('base64').replace(/\++/g, '-').replace(/\/+/g, '_').replace(/=+/g, '')}`)
        ...
        for (let index = 0; index < parseInt(count); index++) {
          ...
          if (name === document.getElementById('searchForName').value) {
            ...
            document.getElementById('transferAddress').addEventListener('keyup', async function (event) {
              if (event.key !== 'Enter') return
              const transferCall = $vite_vitejs.abi.encodeFunctionCall(abi, [document.getElementById('transferAddress').value, index], 'transfer')
              transferingQR.makeCode(`vite:${contractAddress}/transfer?amount=0.01&data=${Buffer.from(transferCall, 'hex').toString('base64').replace(/\++/g, '-').replace(/\/+/g, '_').replace(/=+/g, '')}`)
            })

            document.getElementById('valueTag').addEventListener('keyup', async function (event) {
              if (event.key !== 'Enter') return
              const editCall = $vite_vitejs.abi.encodeFunctionCall(abi, [index, document.getElementById('tagId').value, document.getElementById('valueTag').value], 'editTag')
              editingQR.makeCode(`vite:${contractAddress}/transfer?amount=0.01&data=${Buffer.from(editCall, 'hex').toString('base64').replace(/\++/g, '-').replace(/\/+/g, '_').replace(/=+/g, '')}`)
            })
...
async function updateNameCount () {
  const totalMints = await requestOffCData('getTotalMints', [])
  document.getElementById('nameCount').textContent = `${totalMints}`
}
...
async function startListening () {
  ...
  abi.forEach(f => {
    ...
    signatures[$vite_vitejs.abi.encodeLogSignature(f)] = f
  })
