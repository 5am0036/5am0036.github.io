        ...
        for (let index = 0; index < parseInt(count); index++) {
          ...
          if (name === document.getElementById('searchForName').value) {
            ...
            document.getElementById('qrEditing').onclick = async function () {
              const editCall = $vite_vitejs.abi.encodeFunctionCall(abi, [index, document.getElementById("tagId").options[ document.getElementById("tagId").selectedIndex].value, asciiToHex(document.getElementById('valueTag').value) ], 'editTag')
